...
2. **프로덕션 환경**: Docker 컨테이너 실행 시 환경변수 설정
   ```bash
...
│   ├── Signup.tsx         # 회원가입 페이지
│   ├── GamePage.tsx       # 학습 페이지
│   ├── Progress.tsx       # 진행상황 페이지
│   └── CommunityPage.tsx  # 커뮤니티 페이지
├── store/                 # Redux 상태 관리
│   ├── authSlice.ts       # 인증 상태 (사용자, 토큰, 지갑)
...
- **이전 변경사항**
  ...
  - PrivateRoute 컴포넌트 인증 기반 라우트 보호
